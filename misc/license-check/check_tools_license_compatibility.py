#!/usr/bin/env python3
"""
Tools プロジェクトのMITライセンス互換性チェックスクリプト
Checks MIT license compatibility for the tools project dependencies
"""
...
for package, info in licenses_data.items():
    ...
    # Check if compatible
    elif license_str in MIT_COMPATIBLE or license_str.startswith('(') and all(lic.strip() in MIT_COMPATIBLE for lic in license_str.strip('()').split(' OR ')):
        compatible.append({
            'package': package,
            'license': license_str
        })

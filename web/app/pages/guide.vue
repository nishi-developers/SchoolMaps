<template>
  <div>
    <div class="page">
      <h1>使い方</h1>
      <h2 class="link"><nuxt-link to="/">マップ</nuxt-link></h2>
      <ul>
        <li>
          <Icon icon="mdi:search" />で検索画面を開きます。
        </li>
        <li>
          <Icon icon="material-symbols:reset-focus" />でマップの位置をリセットします。
        </li>
        <li>
          <Icon icon="mdi:label-outline" />で場所名を表示します。
        </li>
        <li>
          <Icon icon="mdi:label-off-outline" />で場所名を非表示します。
        </li>
      </ul>
      <h3>PC・マウス</h3>
      <ul>
        <li>ドラッグで移動します。</li>
        <li>ホイールでズームイン・アウトします。</li>
        <li>ホイールを押しながら左右に動かすことで回転します。</li>
        <li>Shiftキーを押しながら場所をクリックすると、複数選択が可能です。</li>
      </ul>
      <h3>スマホ・タッチ</h3>
      <ul>
        <li>スワイプで移動します。</li>
        <li>ピンチイン・アウトでズームイン・アウトします。</li>
        <li>2本指で回転させて回転します。</li>
      </ul>
      <h2>詳細</h2>
      <ul>
        <li>場所をタップしたら出てくる詳細画面では、場所の情報や画像を見ることができます。</li>
        <li>バーをスワイプまたはタップすることで、詳細画面を拡大・縮小・開閉が可能です。</li>
      </ul>
      <h2 class="link">
        <RouterLink to="search">検索</RouterLink>
      </h2>
      <ul>
        <li>検索ワードを入力することで、場所を検索できます。</li>
      </ul>
      <h2 class="link" @click="installPwa?.()">ホーム画面に追加</h2>
      <ul>
        <li>サイトをホーム画面に追加することで、オフラインでもマップを利用できます。</li>
        <li>ブラウザによっては対応していない場合があります。</li>
      </ul>
    </div>
  </div>
</template>
<script setup lang="ts">
useHead({ title: '使い方' })
const installPwa = useState<() => void>("installPwa")
</script>
<style scoped lang="scss">
.link,
.link a {
  cursor: pointer;
  color: var(--URLBodyColor);
  text-decoration: underline;
}
</style>
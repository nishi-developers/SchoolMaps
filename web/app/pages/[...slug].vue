...
let Setup = new class {
  ...
  changeURL(floor, id, layer) {
    ...
    let url = `${useRuntimeConfig().app.baseURL}${floor}`
    if (id != "" && id != null) {
      url += `/${id}`
    }
    if (layer != "" && layer != null) {
  ...
  resolveUrl() {
    ...
    let id
    let layer
    if (useRuntimeConfig().app.baseURL != "/") {
      // ベースURLがある場合
...
// const { analyticsProxy } = useScriptGoogleAnalytics()
let Property = new class {
  ...
  show(id) {
    // analyticsProxy.gtag("event", `open{${currentFloor.value}/${id}}`)
